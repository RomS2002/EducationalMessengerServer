...
public class ComparatorListGroupDTO implements Comparator<GroupDTO> {
	...
	public int compare(GroupDTO group1, GroupDTO group2) {
		...
		try {
			...
			if (sdf.parse(group1.getLastMessageDate())
					.before(sdf.parse(group2.getLastMessageDate()))) {
				return 1;
			} else if (sdf.parse(group1.getLastMessageDate())
					.after(sdf.parse(group2.getLastMessageDate()))) {
				return -1;
			} else {
				return 0;
			}
