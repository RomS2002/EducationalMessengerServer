...
public class StorageService {
	...
	public void store(MultipartFile file, int messageId) {
		String completeName = StringUtils
		...
		String uri = ServletUriComponentsBuilder.fromCurrentContextPath().path("/uploads/")
		...
		try {
			...
			if (fileName.contains("..")) {
				log.warn("Cannot store file with relative path outside current directory {}",
						newName);
			}
			try (InputStream inputStream = file.getInputStream()) {
				Files.copy(inputStream,
						Paths.get(StaticVariable.FILE_STORAGE_PATH).resolve(newName),
